 ...
 * @brief Command-line interface for vector operations using SIMD-optimized
 * assembly
 *
 ...
 * @code
 *   # Cosine similarity
 *   echo '{"op":"cosine","vec1":[1,2,3],"vec2":[4,5,6],"dim":3}' |
 * ./vector_ops_cli
 *
 *   # Dot product
 *   echo '{"op":"dot","vec1":[1,2,3],"vec2":[4,5,6],"dim":3}' |
 * ./vector_ops_cli
 *
 *   # Vector norm
 ...
 *
 * Input format:
 * - JSON object with fields: "op" (operation name), "vec1" (array), "vec2"
 * (optional array), "dim" (dimension)
 * - Dimension can be omitted if vec1 array is provided (will be auto-detected)
 *
 * Output format:
 * - Success: JSON object with "result" field (scalar for cosine/dot/norm, array
 * for normalize)
 ...
 *
 * @note This tool uses a simplified JSON parser. For production use, consider
 * cJSON library.
 ...
 *
 * Parses a simplified JSON format to extract operation type, vectors, and
 * dimension. This is a basic parser suitable for CLI usage. For production
 * applications, use a proper JSON library like cJSON.
 *
 * Input JSON format:
 * @code
 * {
 ...
 * @param vec1 Output array for first vector (must be pre-allocated, size >=
 * MAX_DIMENSION)
 * @param vec2 Output array for second vector (must be pre-allocated, size >=
 * MAX_DIMENSION)
 ...
 */
int parse_json_simple(char *json, char *op, float *vec1, float *vec2,
                      int *dim) {
