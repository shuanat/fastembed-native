 ...
 * @return NumPy array with embedding vector
 */
py::array_t<float> generate_onnx_embedding(const std::string &model_path,
  ...
  // Call C function
  int status =
      fastembed_onnx_generate(model_path.c_str(), text.c_str(), ptr, dimension);
 ...
 */
int unload_onnx_model() { return fastembed_onnx_unload(); }
 ...
 */
float cosine_similarity(py::array_t<float> vector_a,
                        py::array_t<float> vector_b) {
 ...
 */
py::array_t<float> add_vectors(py::array_t<float> vector_a,
                               py::array_t<float> vector_b) {
  py::buffer_info buf_a = vector_a.request();
  py::buffer_info buf_b = vector_b.request();
...
public:
  ...
  float vector_norm(py::array_t<float> vector) { return ::vector_norm(vector); }
  ...
  int unload_onnx_model() { return ::unload_onnx_model(); }
...
PYBIND11_MODULE(fastembed_native, m) {
  m.doc() = "FastEmbed native extension module for high-performance embedding "
            "generation";

  // Module-level functions
  m.def("generate_embedding", &generate_embedding,
        "Generate embedding from text", py::arg("text"),
        py::arg("dimension") = 768);

  m.def("cosine_similarity", &cosine_similarity,
        "Calculate cosine similarity between two vectors", py::arg("vector_a"),
        py::arg("vector_b"));

  m.def("dot_product", &dot_product, "Calculate dot product of two vectors",
        py::arg("vector_a"), py::arg("vector_b"));

  m.def("vector_norm", &vector_norm, "Calculate L2 norm of a vector",
        py::arg("vector"));

  m.def("normalize_vector", &normalize_vector,
        "Normalize vector (L2 normalization)", py::arg("vector"));

  m.def("add_vectors", &add_vectors, "Add two vectors element-wise",
        py::arg("vector_a"), py::arg("vector_b"));

  m.def("generate_onnx_embedding", &generate_onnx_embedding,
        "Generate ONNX embedding from text", py::arg("model_path"),
        py::arg("text"), py::arg("dimension") = 768);
  ...
  py::class_<FastEmbedNative>(m, "FastEmbedNative")
      .def(py::init<int>(), "Initialize FastEmbed with specified dimension",
           py::arg("dimension") = 768)
      .def("generate_embedding", &FastEmbedNative::generate_embedding,
           "Generate embedding from text", py::arg("text"))
      .def("cosine_similarity", &FastEmbedNative::cosine_similarity,
           "Calculate cosine similarity", py::arg("vector_a"),
           py::arg("vector_b"))
      .def("dot_product", &FastEmbedNative::dot_product,
           "Calculate dot product", py::arg("vector_a"), py::arg("vector_b"))
      .def("vector_norm", &FastEmbedNative::vector_norm,
           "Calculate vector norm", py::arg("vector"))
      .def("normalize_vector", &FastEmbedNative::normalize_vector,
           "Normalize vector", py::arg("vector"))
      .def("add_vectors", &FastEmbedNative::add_vectors, "Add two vectors",
           py::arg("vector_a"), py::arg("vector_b"))
