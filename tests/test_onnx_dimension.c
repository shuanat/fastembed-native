 ...
 *
 * Compile: gcc -o test_onnx_dimension test_onnx_dimension.c -L../build -lfastembed -lm -I../include -DUSE_ONNX_RUNTIME
 * Run: LD_LIBRARY_PATH=.. ./test_onnx_dimension
 */
...
void test_dimension_auto_detection()
{
    ...
    const char *model_path = "models/test.onnx"; /* Placeholder - adjust as needed */
...
#else
    printf("  ⚠ SKIP: ONNX Runtime not available (compiled without USE_ONNX_RUNTIME)\n");
    tests_run++;
...
void test_dimension_validation()
{
    ...
    int result_correct = fastembed_onnx_generate(model_path, "Test text", output_correct, model_dimension);
    ...
    int result_wrong = fastembed_onnx_generate(model_path, "Test text", output_wrong, wrong_dimension);
    ASSERT_NE_INT(result_wrong, 0); /* Should fail */
    printf("  ✓ PASS: Dimension mismatch correctly rejected (wrong dimension: %d)\n", wrong_dimension);
    free(output_wrong);
    ...
    int result_auto = fastembed_onnx_generate(model_path, "Test text", output_auto, 0);
...
void test_supported_dimensions()
{
    ...
    {
        printf("  ⚠ WARNING: Model dimension %d is not in standard supported list\n", model_dimension);
    }
#else
    printf("  ⚠ SKIP: ONNX Runtime not available (compiled without USE_ONNX_RUNTIME)\n");
    tests_run++;
