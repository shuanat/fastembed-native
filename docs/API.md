...
**Navigation**: [Documentation Index](README.md) → API Reference
...
Generate hash-based embedding from text using improved Square Root normalization algorithm.

**Parameters:**

- `text` - Input text string (null-terminated, max 8192 chars)
- `output` - Output float array (must be pre-allocated to dimension size)
- `dimension` - Embedding dimension (0 for default 128, or 128/256/512/768/1024/2048)

**Returns:** `0` on success, `-1` on error (null pointer, invalid dimension, text too long)

**Notes:**

- Default dimension is 128 (use `dimension=0` or `dimension=128`)
- For BERT compatibility, use `dimension=768` explicitly
- Output embedding is L2-normalized (unit vector)
- Supported dimensions: 128, 256, 512, 768, 1024, 2048

---

#### `fastembed_batch_generate`

```c
int fastembed_batch_generate(const char** texts, int num_texts, float** outputs, int dimension);
```

Generate embeddings for multiple texts in a single batch operation. More efficient than calling `fastembed_generate()` multiple times.

**Parameters:**

- `texts` - Array of input text strings (null-terminated, max 8192 chars each)
- `num_texts` - Number of texts in the array
- `outputs` - Array of output float arrays (each must be pre-allocated to dimension size)
- `dimension` - Embedding dimension (0 for default 128, or 128/256/512/768/1024/2048)
...
- All texts are processed with the same dimension
- On error, some embeddings may have been generated (partial results)
- More efficient than individual `fastembed_generate()` calls
- Each output embedding is L2-normalized (unit vector)
- Supported dimensions: 128, 256, 512, 768, 1024, 2048

**Example:**

```c
const char* texts[] = {"Hello world", "Another text", "Third text"};
float* outputs[3];
for (int i = 0; i < 3; i++) {
    outputs[i] = (float*)malloc(128 * sizeof(float));
}
int result = fastembed_batch_generate(texts, 3, outputs, 128);
if (result == 0) {
    // All embeddings generated successfully
    for (int i = 0; i < 3; i++) {
        // Use outputs[i] (128-dimensional embedding)
    }
}
// Clean up
for (int i = 0; i < 3; i++) {
...
Normalize vector to unit length (L2 normalization) in-place.

**Parameters:**

- `vec` - Input/output vector (modified in-place)
- `dimension` - Vector dimension

**Returns:** None (modifies `vec` in-place)

**Notes:**

- Normalizes vector to L2 norm = 1.0 (unit vector)
- Handles zero vectors safely (avoids division by zero)
- Performance: O(dimension), optimized with SIMD
...
#### `generateOnnxEmbedding(modelPath, text, dimension?)`

```javascript
const embedding = client.generateOnnxEmbedding(modelPath, text, dimension);
```

Generate embedding using ONNX Runtime model. Loads a trained ONNX embedding model (e.g., BERT-based, nomic-embed-text) and generates embeddings using neural network inference.

- **Parameters:**
  - `modelPath` (string) - Path to .onnx model file (must be readable)
  - `text` (string) - Input text to embed (max 8192 chars)
  - `dimension` (number, optional) - Embedding dimension (must match model output, max 2048). If 0 or undefined, automatically detects dimension from model.
- **Returns:** `Float32Array` - Embedding vector (L2-normalized, unit vector)
- **Throws:** `Error` - If model not found, inference fails, dimension mismatch, or ONNX Runtime unavailable

**Notes:**

- Requires ONNX Runtime to be installed and linked at compile time
- Model is cached after first load - use `unloadOnnxModel()` to free memory
- Dimension is automatically validated against model output
- Output embedding is L2-normalized (unit vector)
...
```javascript
// Auto-detect dimension
const embedding = client.generateOnnxEmbedding("model.onnx", "text");

// Or specify dimension explicitly (must match model)
const embedding = client.generateOnnxEmbedding("model.onnx", "text", 768);
```
...
```javascript
const result = client.unloadOnnxModel();
```
...
- **Notes:**
  - Safe to call even if no model is loaded (returns 0)
  - After calling this function, the next call to `generateOnnxEmbedding()` will automatically reload the model
...
```javascript
const errorMessage = client.getOnnxLastError();
```

Get last error message from ONNX operations. Returns the last error message that occurred during ONNX operations.

- **Returns:** `string | null` - Error message string, or `null` if no error message available
- **Notes:**
  - Only available when compiled with `USE_ONNX_RUNTIME`
  - Error message is cleared on each new ONNX operation
...
try {
  ...
  const lastError = client.getOnnxLastError();
  console.error("ONNX error:", lastError || error.message);
}
```
...
#### `generate_onnx_embedding(model_path, text, dimension=768)`

```python
embedding = client.generate_onnx_embedding(model_path, text, dimension)
```

Generate embedding using ONNX Runtime model. Loads a trained ONNX embedding model (e.g., BERT-based, nomic-embed-text) and generates embeddings using neural network inference.

- **Parameters:**
  - `model_path` (str) - Path to .onnx model file (must be readable)
  - `text` (str) - Input text to embed (max 8192 chars)
  - `dimension` (int, optional) - Embedding dimension (must match model output, max 2048). Default: 768. If 0, automatically detects dimension from model.
- **Returns:** `numpy.ndarray` - Embedding vector (dtype=float32, L2-normalized, unit vector)
- **Raises:** `RuntimeError` - If model not found, inference fails, dimension mismatch, or ONNX Runtime unavailable

**Notes:**

- Requires ONNX Runtime to be installed and linked at compile time
- Model is cached after first load - use `unload_onnx_model()` to free memory
- Dimension is automatically validated against model output
- Output embedding is L2-normalized (unit vector)
...
```python
# Auto-detect dimension (pass 0)
embedding = client.generate_onnx_embedding("model.onnx", "text", 0)

# Or specify dimension explicitly (must match model)
embedding = client.generate_onnx_embedding("model.onnx", "text", 768)
```
...
```python
result = client.unload_onnx_model()
```
...
- **Notes:**
  - Safe to call even if no model is loaded (returns 0)
  - After calling this function, the next call to `generate_onnx_embedding()` will automatically reload the model
...
```csharp
float[] embedding = client.GenerateOnnxEmbedding(modelPath, text);
```
...
- **Parameters:**
  - `modelPath` (string) - Path to .onnx model file (must be readable)
  - `text` (string) - Input text to embed (max 8192 chars)
- **Returns:** `float[]` - Embedding vector (L2-normalized, unit vector)
- **Throws:**
  - `ArgumentNullException` - If modelPath or text is null
  - `FastEmbedException` - If model not found, inference fails, dimension mismatch, or ONNX Runtime unavailable

**Notes:**

- Requires ONNX Runtime to be installed and linked at compile time
- Uses client's dimension (set in constructor) - must match model output dimension
- Model is cached after first load - use `UnloadOnnxModel()` to free memory
- Dimension is automatically validated against model output
- Output embedding is L2-normalized (unit vector)
...
```csharp
// Create client with dimension matching ONNX model
var client = new FastEmbedClient(768); // 768 for BERT models

// Generate ONNX embedding
float[] embedding = client.GenerateOnnxEmbedding("model.onnx", "text");
```
...
- **Returns:** `float[]` - Embedding vector (L2-normalized, unit vector)
- **Throws:**
  - `IllegalArgumentException` - If modelPath or text is null
  - `FastEmbedException` - If model not found, inference fails, dimension mismatch, or ONNX Runtime unavailable
...
```java
// Create client with dimension matching ONNX model
FastEmbed client = new FastEmbed(768); // 768 for BERT models
...
- **[Architecture Documentation](ARCHITECTURE.md)** - System architecture, data flow, and component design
- **[Use Cases](USE_CASES.md)** - Real-world scenarios and practical examples
- **[Algorithm Specification](ALGORITHM_SPECIFICATION.md)** - Hash-based embedding algorithm details
- **[Algorithm Math](ALGORITHM_MATH.md)** - Mathematical foundation
